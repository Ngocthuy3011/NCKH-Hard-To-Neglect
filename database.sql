...
--Lớp học có những sinh viên nào đăng kí
CREATE TABLE enrollments (
    student_id VARCHAR(20) NOT NULL, -- Mã số sinh viên
    class_id INT NOT NULL,           -- Khóa ngoại liên kết với bảng classes
...
CREATE TABLE attendance (
    id INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY, -- PK, NN, AI
    student_id VARCHAR(20) NOT NULL,                 -- Mã số sinh viên
    class_id INT NOT NULL,                          -- ID lớp học (phải khớp kiểu dữ liệu INT với bảng classes)
    time TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, -- Thời điểm điểm danh
    status VARCHAR(20) NOT NULL DEFAULT 'vắng',      -- Trạng thái (Ví dụ: có mặt, muộn, vắng)
